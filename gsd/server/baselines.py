...
if __name__ == "__main__":
    ...
    if (mode == 'cv'):
        ...
        kf = cross_validation.StratifiedKFold(y=Y, n_folds=cv_folds, shuffle=True, random_state=None)
        ...
        for train_index, test_index in kf:
            ...
            logloss.append(metrics.log_loss(Y_test, Y_pred_prob))
            accuracy.append(metrics.accuracy_score(Y_test, Y_pred_label))

            msg = "fold " + str(len(logloss)) + ": logloss=" + str(logloss[-1]) + "; accuracy=" + str(accuracy[-1]) + "\n"
            print msg
            exp_log.write(msg)

        msg = "average: logloss=" + str(np.mean(logloss)) + "; accuracy=" + str(np.mean(accuracy))
        print msg
        exp_log.write(msg)
